...
pub trait WSpaceReaderZipper<'s> : ZipperMoving + ZipperReadOnlyConditionalValues<'s, f32> + ZipperReadOnlyConditionalIteration<'s, f32> + ZipperReadOnlySubtries<'s, f32> + ZipperIteration + Catamorphism<f32> + ZipperAbsolutePath + ZipperPathBuffer + ZipperForking<f32> {}
impl<'s, T> WSpaceReaderZipper<'s> for T where T : ZipperMoving + ZipperReadOnlyConditionalValues<'s, f32> + ZipperReadOnlyConditionalIteration<'s, f32> + ZipperReadOnlySubtries<'s, f32> + ZipperIteration + Catamorphism<f32> + ZipperAbsolutePath + ZipperPathBuffer + ZipperForking<f32> {}

pub trait WSpaceWriterZipper : ZipperMoving + ZipperWriting<f32> + ZipperForking<f32> + ZipperAbsolutePath {}
impl<T> WSpaceWriterZipper for T where T : ZipperMoving + ZipperWriting<f32> + ZipperForking<f32> + ZipperAbsolutePath {}
...
pub trait WPermissionArb<'space, S: WSpace> {
    /// Requests a new [Space::Reader] from the `Space`
    fn new_reader(&self, path: &[u8], auth: &S::Auth) -> Result<S::Reader<'space>, S::PermissionErr>;
...
pub trait WSpace: Sized {
    ...
    type Writer<'space> where Self: 'space;
    /// An object from which reader and writer permissions may be acquired within the space
    type PermissionHead<'space>: WPermissionArb<'space, Self> where Self: 'space;
    /// An error type for when a new reader or writer cannot be authenticated.
    type PermissionErr: PathPermissionErr;
