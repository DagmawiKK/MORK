        ...
        match stack.pop() {
            ...
            Some(e) => {
                ...
                match byte_item(e_byte) {
                    ...
                    Tag::LongVarRef => {
                        ...
                        let addition = if e.n == 0 {
                            ...
                            ExprEnv {
                                ...
                                offset: 0,
                                base: Expr {
                                    ptr: loc.path().as_ptr().cast_mut().offset(references[i] as _),
                                },
                            }
                        } else {
                    ...
                    Tag::LongArity => {
                        let a = unsafe {
                            mork_expr::read_arity_at(e.base.ptr.byte_add(e.offset as usize))
                        };
                        vs!(e, false);
                        if loc.descend_to_existing_byte(e_byte) {
                            let stackl = stack.len();
...
macro_rules! expr {
    ($space:ident, $s:literal) => {{
        ...
        let used = q.substitute_symbols(&mut mork_expr::ExprZipper::new(p), |x| {
            <_ as mork_frontend::bytestring_parser::Parser>::tokenizer(&mut pdp, x)
        });
        unsafe {
...
impl Space {
    ...
    pub fn query_multi_i<F: FnMut(Result<&[u32], BTreeMap<(u8, u8), ExprEnv>>, Expr) -> bool>(
        no_source: bool,
        mut mmaps: &mut HashMap<&'static str, ArenaCompactTree<memmap2::Mmap>>,
        btm: &PathMap<()>,
        pat_expr: Expr,
        mut effect: F,
    ) -> usize {
        ...
        for e in pat_args[1..].iter() {
            ...
            factors.push(src.source(src.request().map(|request| match request {
                ResourceRequest::BTM(prefix) => Resource::BTM(btm.read_zipper_at_path(prefix)),
                ResourceRequest::ACT(name) => {
                    let act = unsafe { mmaps_ptr.as_mut().unwrap() }
                        .entry(name)
                        .or_insert_with(|| {
    ...
    pub fn transform_multi_multi_o(
        ...
        let mut zh = self.btm.zipper_head();
        read_copy.insert(unsafe { add.span().as_ref().unwrap() }, ());
        let mut template_resources: Vec<WriteResource<'_, '_, '_>> = Vec::with_capacity(64);
        let mut outstanding_wzs = Vec::with_capacity(64);
        let mut outstanding_wzs_ptr =
                    ...
                    WriteResourceRequest::BTM(p) => {
                        unsafe {
                            outstanding_wzs_ptr
                                .as_mut()
                                .unwrap()
                                .push(zh.write_zipper_at_exclusive_path_unchecked(p))
                        };
                        template_resources.push(WriteResource::BTM(unsafe {
                            outstanding_wzs_ptr.as_mut().unwrap().last_mut().unwrap()
                        }))
                    }
                    WriteResourceRequest::ACT(f) => template_resources.push(WriteResource::ACT(())),
    ...
    pub fn metta_calculus(&mut self, steps: usize) -> usize {
        ...
        while {
            ...
            if rz.to_next_val() {
                ...
                if let Err(e) = self.interpret(Expr {
                    ptr: x.as_mut_ptr(),
                }) {
