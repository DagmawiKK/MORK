...
pub struct SpaceTranscriber<'a, 'b, 'c> { count: usize, wz: &'c mut WriteZipperUntracked<'a, 'b, i32>, pdp: ParDataParser<'a> }
...
impl Space {
    ...
    pub fn restore_paths<OutDirPath : AsRef<std::path::Path>>(&mut self, path: OutDirPath) -> Result<pathmap::paths_serialization::DeserializationStats, std::io::Error> {
        ...
        pathmap::paths_serialization::deserialize_paths(self.btm.write_zipper(), &mut file, 0)
    }

    pub fn query_multi<F : FnMut(Result<&[u32], BTreeMap<(u8, u8), ExprEnv>>, Expr) -> bool>(btm: &PathMap<i32>, pat_expr: Expr, mut effect: F) -> usize {
    ...
    pub fn query_multi_i<F : FnMut(Result<&[u32], BTreeMap<(u8, u8), ExprEnv>>, Expr) -> bool>(no_source: bool, mut mmaps: &mut HashMap<&'static str, ArenaCompactTree<memmap2::Mmap>>, btm: &PathMap<i32>, pat_expr: Expr, mut effect: F) -> usize {
    ...
    pub fn transform_multi_multi_(&mut self, pat_expr: Expr, tpl_expr: Expr, add: Expr) -> (usize, bool) {
        ...
        read_copy.insert(unsafe { add.span().as_ref().unwrap() }, 0);
