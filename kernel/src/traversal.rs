...
use log::trace;
use pathmap::{morphisms::Catamorphism, zipper::{ReadZipperTracked, ReadZipperUntracked, Zipper, ZipperAbsolutePath, ZipperForking, ZipperMoving, ZipperValues}};
...
pub fn next_atom(mut z: ReadZipperTracked<i32>) -> Option<Vec<u8>> {

    // 1) choose random number
    let mut root_agg_w = node_agg_w(z.fork_read_zipper()).unwrap();
    ...
    while z.child_count() >= 1 {
        ...
        let choice = choice(&mut random_num, choice_param.to_vec(), &z.fork_read_zipper());
...
fn choice(random_num: &mut i32, mut choice_param: Vec<(i32, u8)>, path: &ReadZipperUntracked<i32>) -> PathChoice {
    ...
    if path.val().is_some() {
        ...
        if path_v_val(&focus_val, &choice_param, random_num) {
...
fn path_v_val(focus_val: &i32, choice_param: &Vec<(i32, u8)>, random_num: &mut i32) -> bool {
