...
pub(crate) enum Resource<'trie, 'path> {
    BTM(ReadZipperUntracked<'trie, 'path, i32>),
    ACT(ACTMmapZipper<'trie, i32>)
}
...
pub trait Source {
    ...
    fn source<'trie, 'path, It : Iterator<Item=Resource<'trie, 'path>>>(&self, it: It) -> AFactor<'trie, i32> where 'path : 'trie;
}
...
impl CmpSource {
    fn policy(ctx: (usize, PathMap<i32>), p: &[u8], c: usize) -> ((usize, PathMap<i32>), Option<ReadZipperOwned<i32>>) {
        let (cmp, map) = ctx;
        if c == 0 {
            if cmp == 0 {
                ...
                ((cmp, map), Some(PathMap::single(&qv[..], 0).into_read_zipper(&[])))
            } else if cmp == 1 {
...
impl Source for CmpSource {
    ...
    fn source<'trie, 'path, It: Iterator<Item=Resource<'trie, 'path>>>(&self, mut it: It) -> AFactor<'trie, i32> where 'path : 'trie {
        ...
        let rz = DependentProductZipperG::new_enroll(rz, (self.cmp, map),
            CmpSource::policy as for<'a> fn((usize, PathMap<i32>), &'a [u8], usize) -> ((usize, PathMap<i32>), Option<ReadZipperOwned<i32>>));
